	...
	public Dynamic<?> fixTag(Dynamic<Tag> dynamic) {
		...
		if (optional.isEmpty()) {
			...
			if (optional2.isEmpty()) {
				...
				Identifier identifier = (Identifier)TrialSpawnerConfigInRegistryFix.VanillaTrialChambers.CONFIGS_TO_KEY
					.get(Pair.of((Dynamic)optional.get(), (Dynamic)optional2.get()));
				return identifier == null
					? dynamic
					: dynamic.set("normal_config", dynamic.createString(identifier.withSuffix("/normal").toString()))
						.set("ominous_config", dynamic.createString(identifier.withSuffix("/ominous").toString()));
			}
		}
	}
	...
	static final class VanillaTrialChambers {
		...
		private static void register(Identifier identifier, String string, String string2) {
			try {
				...
				CONFIGS_TO_KEY.put(Pair.of(dynamic, asDynamic(compoundTag2)), identifier);
				CONFIGS_TO_KEY.put(Pair.of(dynamic, asDynamic(compoundTag3)), identifier);
				CONFIGS_TO_KEY.put(Pair.of(dynamic, asDynamic(compoundTag4)), identifier);
