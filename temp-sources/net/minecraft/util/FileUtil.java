...
public class FileUtil {
	...
	public static DataResult<List<String>> decomposePath(String string) {
		...
		if (i == -1) {
			return switch (string) {
				...
				default -> !containsAllowedCharactersOnly(string) ? DataResult.error(() -> "Invalid path '" + string + "'") : DataResult.success(List.of(string));
			};
		} else {
			...
			while (true) {
				...
				if (!containsAllowedCharactersOnly(string2)) {
					return DataResult.error(() -> "Invalid segment '" + string2 + "' in path '" + string + "'");
				}
	...
	public static boolean isValidPathSegment(String string) {
		return !string.equals("..") && !string.equals(".") && containsAllowedCharactersOnly(string);
	}
