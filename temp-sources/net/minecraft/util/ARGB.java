...
public class ARGB {
	...
	private static float computeSrgbToLinear(float f) {
		return f >= 0.04045F ? (float)Math.pow((f + 0.055) / 1.055, 2.4) : f / 12.92F;
	}

	private static float computeLinearToSrgb(float f) {
		return f >= 0.0031308F ? (float)(1.055 * Math.pow(f, 0.4166666666666667) - 0.055) : 12.92F * f;
	}

	public static float srgbToLinearChannel(int i) {
		return SRGB_TO_LINEAR[i] / 1023.0F;
	}
	...
	public static int linearLerp(float f, int i, int j) {
		return color(
			...
			LINEAR_TO_SRGB[Mth.lerpInt(f, SRGB_TO_LINEAR[red(i)], SRGB_TO_LINEAR[red(j)])] & 0xFF,
			LINEAR_TO_SRGB[Mth.lerpInt(f, SRGB_TO_LINEAR[green(i)], SRGB_TO_LINEAR[green(j)])] & 0xFF,
			LINEAR_TO_SRGB[Mth.lerpInt(f, SRGB_TO_LINEAR[blue(i)], SRGB_TO_LINEAR[blue(j)])] & 0xFF
		);
	}
