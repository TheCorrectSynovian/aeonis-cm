		...
		if (dynamicOps instanceof RegistryOps<?> registryOps) {
			...
			if (optional.isPresent()) {
				...
				return holder.unwrap()
					.map(
						resourceKey -> Identifier.CODEC.encode(resourceKey.identifier(), dynamicOps, object), object2 -> this.elementCodec.encode((E)object2, dynamicOps, object)
					);
			}
		}
	...
	public <T> DataResult<Pair<Holder<E>, T>> decode(DynamicOps<T> dynamicOps, T object) {
		if (dynamicOps instanceof RegistryOps<?> registryOps) {
			...
			if (optional.isEmpty()) {
				...
				DataResult<Pair<Identifier, T>> dataResult = Identifier.CODEC.decode(dynamicOps, object);
