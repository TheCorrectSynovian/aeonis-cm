...
public class ResourceKeyArgument<T> implements ArgumentType<ResourceKey<T>> {
	...
	public static <T> ResourceKey<T> getRegistryKey(
		...
		return (ResourceKey<T>)optional.orElseThrow(() -> dynamicCommandExceptionType.create(resourceKey2.identifier()));
	}
	...
	private static <T> Holder.Reference<T> resolveKey(
		...
		return (Holder.Reference<T>)getRegistry(commandContext, resourceKey)
			.get(resourceKey2)
			.orElseThrow(() -> dynamicCommandExceptionType.create(resourceKey2.identifier()));
	}
	...
	public static RecipeHolder<?> getRecipe(CommandContext<CommandSourceStack> commandContext, String string) throws CommandSyntaxException {
		...
		return (RecipeHolder<?>)recipeManager.byKey(resourceKey).orElseThrow(() -> ERROR_INVALID_RECIPE.create(resourceKey.identifier()));
	}
