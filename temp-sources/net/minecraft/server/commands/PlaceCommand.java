...
public class PlaceCommand {
											...
											Commands.argument("max_depth", IntegerArgumentType.integer(1, 20))
												.executes(
													commandContext -> placeJigsaw(
														...
														IdentifierArgument.getId(commandContext, "target"),
							...
							Commands.argument("template", IdentifierArgument.id())
								...
								.executes(
									commandContext -> placeTemplate(
										commandContext.getSource(),
										IdentifierArgument.getId(commandContext, "template"),
										BlockPos.containing(commandContext.getSource().getPosition()),
										Rotation.NONE,
										Mirror.NONE,
										1.0F,
	...
	public static int placeTemplate(
		CommandSourceStack commandSourceStack, Identifier identifier, BlockPos blockPos, Rotation rotation, Mirror mirror, float f, int i, boolean bl
		...
		if (optional.isEmpty()) {
			...
			if (!bl2) {
				...
				commandSourceStack.sendSuccess(
					() -> Component.translatable("commands.place.template.success", Component.translationArg(identifier), blockPos.getX(), blockPos.getY(), blockPos.getZ()),
					true
				);
				return 1;
			}
