	...
	private static int listForceLoad(CommandSourceStack commandSourceStack) {
		...
		if (i > 0) {
			...
			if (i == 1) {
				...
				commandSourceStack.sendSuccess(
					() -> Component.translatable("commands.forceload.list.multiple", i, Component.translationArg(resourceKey.identifier()), string), false
				);
			}
		} else {
					...
					if (tx == 1) {
						commandSourceStack.sendSuccess(
							() -> Component.translatable(
								"commands.forceload." + (bl ? "added" : "removed") + ".single", Component.translationArg(chunkPos2), Component.translationArg(resourceKey.identifier())
							),
							true
						);
					} else {
