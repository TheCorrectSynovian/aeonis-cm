...
public class FunctionCommand {
	...
	private static <T extends ExecutionCommandSource<T>> void instantiateAndQueueFunctions(
		...
		try {
			...
			throw ERROR_FUNCTION_INSTANTATION_FAILURE.create(identifier, var9.messageComponent());
		}
	}

	private static <T extends ExecutionCommandSource<T>> CommandResultCallback decorateOutputIfNeeded(
		T executionCommandSource, FunctionCommand.Callbacks<T> callbacks, Identifier identifier, CommandResultCallback commandResultCallback
	) {
	...
	private static <T extends ExecutionCommandSource<T>> void queueFunctionsAsReturn(
		...
		for (CommandFunction<T> commandFunction : collection) {
			...
			CommandResultCallback commandResultCallback2 = decorateOutputIfNeeded(executionCommandSource, callbacks, identifier, commandResultCallback);
			instantiateAndQueueFunctions(
				compoundTag, executionControl, commandDispatcher, executionCommandSource3, commandFunction, identifier, commandResultCallback2, true
			);
		}
	...
	private static <T extends ExecutionCommandSource<T>> void queueFunctionsNoReturn(
		...
		if (!collection.isEmpty()) {
			if (collection.size() == 1) {
				...
				CommandResultCallback commandResultCallback2 = decorateOutputIfNeeded(executionCommandSource, callbacks, identifier, commandResultCallback);
				instantiateAndQueueFunctions(
					compoundTag, executionControl, commandDispatcher, executionCommandSource3, commandFunction, identifier, commandResultCallback2, false
				);
				...
				for (CommandFunction<T> commandFunction2 : collection) {
					...
					CommandResultCallback commandResultCallback3 = decorateOutputIfNeeded(executionCommandSource, callbacks, identifier2, commandResultCallback);
					instantiateAndQueueFunctions(
						compoundTag, executionControl, commandDispatcher, executionCommandSource3, commandFunction2, identifier2, commandResultCallback3, false
					);
				}
			} else {
				class Accumulator {
				...
				for (CommandFunction<T> commandFunction3 : collection) {
					...
					CommandResultCallback commandResultCallback5 = decorateOutputIfNeeded(executionCommandSource, callbacks, identifier3, commandResultCallback4);
					instantiateAndQueueFunctions(
						compoundTag, executionControl, commandDispatcher, executionCommandSource3, commandFunction3, identifier3, commandResultCallback5, false
					);
				}
	...
	public interface Callbacks<T> {
		void signalResult(T object, Identifier identifier, int i);
	}
