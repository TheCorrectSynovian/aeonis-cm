		...
		while (this.server.isRunning()) {
			...
			if (n > this.maxTickTimeNanos) {
				...
				crashReportCategory.setDetail(
					"Random tick rate", (CrashReportDetail<String>)(() -> this.server.getWorldData().getGameRules().getAsString(GameRules.RANDOM_TICK_SPEED))
				);
				crashReportCategory.setDetail(
					"Level stats",
					(CrashReportDetail<String>)(() -> (String)Streams.stream(this.server.getAllLevels())
						.map(serverLevel -> serverLevel.dimension().identifier() + ": " + serverLevel.getWatchdogStats())
