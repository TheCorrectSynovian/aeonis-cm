...
public class TemptingSensor extends Sensor<PathfinderMob> {
	...
	public TemptingSensor(Predicate<ItemStack> predicate) {
		this((BiPredicate<PathfinderMob, ItemStack>)((pathfinderMob, itemStack) -> predicate.test(itemStack)));
	}

	public static TemptingSensor forAnimal() {
		return new TemptingSensor(
			(BiPredicate<PathfinderMob, ItemStack>)((pathfinderMob, itemStack) -> pathfinderMob instanceof Animal animal ? animal.isFood(itemStack) : false)
		);
	}
	...
	protected void doTick(ServerLevel serverLevel, PathfinderMob pathfinderMob) {
		...
		List<Player> list = (List<Player>)serverLevel.players()
			...
			.filter(serverPlayer -> this.playerHoldingTemptation(pathfinderMob, serverPlayer))
