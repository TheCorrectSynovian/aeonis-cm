		...
		if (j >= this.delayTicks) {
			...
			for (EntityHitResult entityHitResult : (Collection)ProjectileUtil.getHitEntitiesAlong(
				...
				if (!bl2) {
					...
					boolean bl3 = this.dismountConditions.isPresent() && ((KineticWeapon.Condition)this.dismountConditions.get()).test(j, d, h, f);
					boolean bl4 = this.knockbackConditions.isPresent() && ((KineticWeapon.Condition)this.knockbackConditions.get()).test(j, d, h, f);
					boolean bl5 = this.damageConditions.isPresent() && ((KineticWeapon.Condition)this.damageConditions.get()).test(j, d, h, f);
					if (bl3 || bl4 || bl5) {
