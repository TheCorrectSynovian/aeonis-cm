...
public class BuiltInLootTables {
	...
	private static ResourceKey<LootTable> register(String string) {
		return register(ResourceKey.create(Registries.LOOT_TABLE, Identifier.withDefaultNamespace(string)));
	}
	...
	private static ResourceKey<LootTable> register(ResourceKey<LootTable> resourceKey) {
		if (LOCATIONS.add(resourceKey)) {
			...
			throw new IllegalArgumentException(resourceKey.identifier() + " is already a registered built-in loot table");
		}
	}
