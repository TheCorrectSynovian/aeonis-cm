...
public record DimensionType(
	...
	public static Path getStorageFolder(ResourceKey<Level> resourceKey, Path path) {
		if (resourceKey == Level.OVERWORLD) {
			...
			return resourceKey == Level.NETHER
				? path.resolve("DIM-1")
				: path.resolve("dimensions").resolve(resourceKey.identifier().getNamespace()).resolve(resourceKey.identifier().getPath());
		}
	}
	...
	public static enum CardinalLightType implements StringRepresentable {
		...
		private CardinalLightType(final String string2) {
			this.name = string2;
		}
	...
	public static enum Skybox implements StringRepresentable {
		...
		private Skybox(final String string2) {
			this.name = string2;
		}
