...
public class JigsawPlacement {
	...
	public static Optional<Structure.GenerationStub> addPieces(
		...
		if (structurePoolElement == EmptyPoolElement.INSTANCE) {
			...
			if (optional.isPresent()) {
				...
				Optional<BlockPos> optional3 = getRandomNamedJigsaw(structurePoolElement, identifier, blockPos, rotation, structureTemplateManager, worldgenRandom);
				if (optional3.isEmpty()) {
					LOGGER.error(
						"No starting jigsaw {} found in start pool {}",
						identifier,
						holder.unwrapKey().map(resourceKey -> resourceKey.identifier().toString()).orElse("<unregistered>")
					);
					return Optional.empty();
				}
	...
	public static boolean generateJigsaw(
		ServerLevel serverLevel, Holder<StructureTemplatePool> holder, Identifier identifier, int i, BlockPos blockPos, boolean bl
	) {
					...
					if (holder.value().size() == 0 && !holder.is(Pools.EMPTY)) {
						...
						if (holder2.value().size() == 0 && !holder2.is(Pools.EMPTY)) {
							JigsawPlacement.LOGGER
								.warn("Empty or non-existent fallback pool: {}", holder2.unwrapKey().map(resourceKeyx -> resourceKeyx.identifier().toString()).orElse("<unregistered>"));
						} else {
								...
								for (Rotation rotation2 : Rotation.getShuffled(this.random)) {
									...
									for (StructureTemplate.JigsawBlockInfo jigsawBlockInfo2 : list2) {
										if (JigsawBlock.canAttach(jigsawBlockInfo, jigsawBlockInfo2)) {
											...
											if (!Shapes.joinIsNotEmpty(mutableObject3.get(), Shapes.create(AABB.of(boundingBox4).deflate(0.25)), BooleanOp.ONLY_SECOND)) {
												mutableObject3.setValue(Shapes.joinUnoptimized(mutableObject3.get(), Shapes.create(AABB.of(boundingBox4)), BooleanOp.ONLY_FIRST));
