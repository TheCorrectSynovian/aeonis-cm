		...
		if (GlRenderPass.VALIDATION) {
			...
			for (Entry<String, Uniform> entry : glRenderPass.pipeline.program().getUniforms().entrySet()) {
				if (entry.getValue() instanceof Uniform.Sampler) {
					...
					GlRenderPass.TextureViewAndSampler textureViewAndSampler = (GlRenderPass.TextureViewAndSampler)glRenderPass.samplers.get(string);
					if (textureViewAndSampler == null) {
						throw new IllegalStateException("Missing sampler " + string);
					}

					GlTextureView glTextureView = textureViewAndSampler.view();
					...
					if (textureViewAndSampler.sampler().isClosed()) {
			...
			switch ((Uniform)entry2.getValue()) {
				case Uniform.Ubo(int var63):
					...
					if (bl2) {
						...
						GL32.glBindBufferRange(35345, var41, ((GlBuffer)gpuBufferSlice2.buffer()).handle, gpuBufferSlice2.offset(), gpuBufferSlice2.length());
					}
					break;
				case Uniform.Utb(int var43, int var44, TextureFormat var45, int var61):
					int var46 = var61;
					if (bl || bl2) {
					...
					if (bl2) {
						...
						GL31.glTexBuffer(35882, GlConst.toGlInternalId(var45), ((GlBuffer)gpuBufferSlice3.buffer()).handle);
					}
					break;
				case Uniform.Sampler(int textureViewAndSampler2, int var53):
					int var48 = var53;
					GlRenderPass.TextureViewAndSampler textureViewAndSampler2x = (GlRenderPass.TextureViewAndSampler)glRenderPass.samplers.get(string2);
					if (textureViewAndSampler2x == null) {
						break;
					}

					GlTextureView glTextureView2 = textureViewAndSampler2x.view();
					if (bl || bl2) {
						GlStateManager._glUniform1i(textureViewAndSampler2, var48);
					}
					...
					GL33C.glBindSampler(var48, textureViewAndSampler2x.sampler().getId());
