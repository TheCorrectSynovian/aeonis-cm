...
public class NautilusModel extends EntityModel<NautilusRenderState> {
	...
	public void setupAnim(NautilusRenderState nautilusRenderState) {
		...
		this.swimAnimation
			.applyWalk(nautilusRenderState.walkAnimationPos + nautilusRenderState.ageInTicks / 5.0F, nautilusRenderState.walkAnimationSpeed + 0.2F, 2.0F, 3.0F);
	}

	private void applyBodyRotation(float f, float g) {
		f = Mth.clamp(f, -10.0F, 10.0F);
		g = Mth.clamp(g, -10.0F, 10.0F);
		this.body.yRot = f * (float) (Math.PI / 180.0);
		this.body.xRot = g * (float) (Math.PI / 180.0);
	}
}
