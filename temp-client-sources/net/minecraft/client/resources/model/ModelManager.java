...
public class ModelManager implements PreparableReloadListener, FabricBakedModelManager {
	...
	public ItemModel getItemModel(Identifier identifier) {
		return (ItemModel)this.bakedItemStackModels.getOrDefault(identifier, this.missingModels.item());
	}

	public ClientItem.Properties getItemProperties(Identifier identifier) {
		return (ClientItem.Properties)this.itemProperties.getOrDefault(identifier, ClientItem.Properties.DEFAULT);
	}
		...
		return CompletableFuture.allOf(
			...
			.thenComposeAsync(
				void_ -> {
					...
					SpriteLoader.Preparations preparations2 = (SpriteLoader.Preparations)completableFuture9.join();
					...
					Set<Identifier> set = Sets.<Identifier>difference(((Map)completableFuture3.join()).keySet(), resolvedModels.models.keySet());
					if (!set.isEmpty()) {
					...
					return loadModels(
						...
						object2IntMap,
						(EntityModelSet)completableFuture.join(),
						(SpecialBlockModelRenderer)completableFuture2.join(),
						executor
					);
				},
				executor
					...
					for (Entry<Identifier, Resource> entry : map.entrySet()) {
						list.add(CompletableFuture.supplyAsync(() -> {
							...
							try {
								...
								Pair var3;
								try {
									var3 = Pair.of(identifier, BlockModel.fromStream(reader));
