...
public class DebugOptionsScreen extends Screen {
	...
	public class OptionList extends ContainerObjectSelectionList<DebugOptionsScreen.AbstractOptionEntry> {
		private static final Comparator<java.util.Map.Entry<Identifier, DebugScreenEntry>> COMPARATOR = (entry, entry2) -> {
			...
			return i != 0 ? i : ((Identifier)entry.getKey()).compareTo((Identifier)entry2.getKey());
		};
		...
		public void updateSearch(String string) {
			this.clearEntries();
			List<java.util.Map.Entry<Identifier, DebugScreenEntry>> list = new ArrayList(DebugScreenEntries.allEntries().entrySet());
			list.sort(COMPARATOR);
			DebugEntryCategory debugEntryCategory = null;

			for (java.util.Map.Entry<Identifier, DebugScreenEntry> entry : list) {
				if (((Identifier)entry.getKey()).getPath().contains(string)) {
