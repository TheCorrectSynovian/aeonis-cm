...
public class ExperimentsScreen extends Screen {
	...
	protected void init() {
		...
		this.packs
			.forEach(
				(ObjectBooleanBiConsumer<? super Pack>)((pack, bl) -> builder.addSwitch(
						getHumanReadableTitle(pack), () -> this.packs.getBoolean(pack), boolean_ -> this.packs.put(pack, boolean_.booleanValue())
					)
	...
	private void onDone() {
		...
		this.packs.forEach((ObjectBooleanBiConsumer<? super Pack>)((pack, bl) -> {
			list.remove(pack);
			if (bl) {
				list2.add(pack);
			}
