	...
	public void render(AbstractContainerMenu abstractContainerMenu, GuiGraphics guiGraphics, float f, int i, int j) {
		...
		if (!this.icons.isEmpty() && !slot.hasItem()) {
			...
			if (g < 1.0F) {
				...
				this.renderIcon(slot, (Identifier)this.icons.get(k), 1.0F - g, guiGraphics, i, j);
			}

			this.renderIcon(slot, (Identifier)this.icons.get(this.iconIndex), g, guiGraphics, i, j);
		}
	}
