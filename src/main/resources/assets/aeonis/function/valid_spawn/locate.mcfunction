execute positioned ~1 ~ ~ store result score Game deeper_dark.distx+1 run locate structure deeper_dark:valid_spawn
execute positioned ~-1 ~ ~ store result score Game deeper_dark.distx-1 run locate structure deeper_dark:valid_spawn
execute positioned ~ ~ ~1 store result score Game deeper_dark.distz+1 run locate structure deeper_dark:valid_spawn
execute positioned ~ ~ ~-1 store result score Game deeper_dark.distz-1 run locate structure deeper_dark:valid_spawn
...
execute if score Game deeper_dark.distx+1 matches 2 if score Game deeper_dark.distz+1 matches 2 run return run function deeper_dark:valid_spawn/end
execute if score Game deeper_dark.distx+1 matches 0 if score Game deeper_dark.distz+1 matches 0 run return fail 
execute as @s positioned as @s run function deeper_dark:valid_spawn/locate